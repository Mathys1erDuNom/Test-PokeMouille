...
# script_dir doit pointer sur la racine du projet, pas le dossier BDD
...
async def tick_chenil_xp(
    ...
    for uid, checks in list(xp_counters.items()):
        ...
        if chenil_data["is_egg"]:
            ...
            # Relit les valeurs fraîches depuis le module BDD
            egg_info = get_egg_info(str(uid))
            current_xp, xp_evo = egg_info if egg_info else (0, 400)
